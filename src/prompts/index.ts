...
// Prompt 模板（参见开发文档第 8 节 Agent 设计原则）。
// 关键约束：LLM 只负责理解任务和生成内容，必须输出结构化 JSON；
// 不允许输出 JS、DOM selector、Cookie 操作等。
...
4. platform 必须是: xiaohongshu | douyin | wechat_channel | wechat_official 之一。
...
7. 如果是发布任务，actions 通常包含: check_login, generate_content, open_publish_page, upload_media(若有素材), fill_title, fill_body, fill_hashtags, submit_publish, verify_result, take_screenshot, save_record。
